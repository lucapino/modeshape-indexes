 ...
 */
package com.github.lucapino.modeshape.indexes.elasticsearch;
 ...
 */
@Test
public class ModeshapeElasticsearchIndexesTest {

    private static final Logger LOGGER = LoggerFactory.getLogger(ModeshapeElasticsearchIndexesTest.class);

    private org.elasticsearch.node.Node esNode;
    ...
    @BeforeClass
    public void startES() {
        logHeader("Start Elasticsearch");
        // Clear repository folder
        FileUtil.delete("target/repoES");
        Settings localSettings = Settings.settingsBuilder()
                .put("http.enabled", true)
                .put("number_of_shards", 1)
                .put("number_of_replicas", 1)
                .put("path.home", "target/repoES/data")
                .build();
    ...
    @AfterClass
    public void stopES() {
        logHeader("Stop Elasticsearch");
        // Stop embedded Elasticsearch
        esNode.close();
    }
    ...
    private void startEngineAndRepository() throws Exception {
        ...
        URL url = this.getClass().getClassLoader().getResource("repositoryElasticsearch.json");
    ...
    private void createCustomIndex(IndexManager indexManager, String workspaceName, String indexName, IndexDefinition.IndexKind indexKind, boolean synchronous, String nodeType, Map<String, Integer> columns) throws RepositoryException {
        ...
        indexDefinitionTemplate.setProviderName("elasticsearch");
    ...
    private void displayIndexes() {
        ...
        for (IndexDefinition def : indexDefinitions.values()) {
            ...
            IndexManager.IndexStatus indexStatus = indexManager.getIndexStatus("elasticsearch", indexName, "default");
    ...
    private void showQueryPlan() throws Exception {
        ...
        Query query = queryManager.createQuery("SELECT * FROM [nt:base] AS base WHERE base.[my:testProperty] ='Test'", Query.JCR_SQL2);
        ...
        query = queryManager.createQuery("SELECT * FROM [nt:base] AS base WHERE base.[jcr:name] ='Test'", Query.JCR_SQL2);
